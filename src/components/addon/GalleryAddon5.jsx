...
import { galleryAddonSet5 } from "../data/GalleryAddonSet";
...
import staticGalleryImage from "../../assets/gallery/staticGalleryAddon5.jpg";
import galleryCircle from "../../assets/other/circle.png";

const GalleryAddon5 = ({ title }) => {
  ...
  const goToPrevSlide = () => {
    const newIndex =
      (currentIndex - 1 + galleryAddonSet5.length) % galleryAddonSet5.length;
    setCurrentIndex(newIndex);
    setDeltaX(0);
  };

  const goToNextSlide = () => {
    const newIndex = (currentIndex + 1) % galleryAddonSet5.length;
    setCurrentIndex(newIndex);
    setDeltaX(0);
  };
        ...
        <section className={styles.galleryCarousel}>
          ...
          >
            <div className={styles.carouselInner}>
              ...
              >
                {galleryAddonSet5.map((image, index) => (
                  <figure
                    key={index}
                    tabIndex="0"
            ...
            >
              ...
              >
                <a
                  href={galleryAddonSet5[currentIndex].link}
                  onClick={(e) => {
                    if (currentPath === galleryAddonSet5[currentIndex].link) {
                      e.preventDefault();
                      return;
                    }
                    trackElementClickEvent(
                      "carousel_link",
                      galleryAddonSet5[currentIndex].title,
                      galleryAddonSet5[currentIndex].link
                    );
                  }}
                  aria-label={`Navigera till ${galleryAddonSet5[currentIndex].title}`}
                >
                  <h2>{galleryAddonSet5[currentIndex].title}</h2>
                </a>
                <p>{galleryAddonSet5[currentIndex].text}</p>
                <div className={styles.linkContainer}>
                  <a
                    ...
                    aria-label={`Läs mer om ${galleryAddonSet5[currentIndex].title}`}
                  >
                    Läs Mer
                  </a>
                </div>
